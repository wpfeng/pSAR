...
###############################################################################
#
def info_from_grd(ingrd,azi=-12,inc=32,wavelength=0.055246,inrsc=None):#
    #
    if inrsc is not None and os.path.exists(inrsc):
        info,_ = rsc_to_info(inrsc)
    else:
        info,_ = roipac_info()
        info['INCIDENCE'] = inc
    ...
    #
    width, height,x0,y0,x_res, y_res, projection = grdinfo(ingrd)
    #
    info['FILE_LENGTH'] = int(height)
...
def grdinfo(ingrd):
    ...
    dataset = gdal.Open(ingrd)
    if dataset is None:
        print(f"Cannot open netcdf file: {file_path}")
        return
    ...
    #
    #
    return width, height,x0,y0,x_res, y_res, projection
