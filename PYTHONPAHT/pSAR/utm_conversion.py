...
#
def ll2local(lats,lons,refll):
    #
    # refll, [lat0,lon0]
    #
    x0,y0,un,ul = from_latlon(refll[0],refll[1]) #, force_zone_number=None,force_zone_letter=None)
    #
    x,y = lltoutm(lats,lons,force_zone_number=un,force_zone_letter=ul)
    return x-x0,y-y0
#
def local2ll(xs,ys,refll):
    # xs (eat) in km
    # ys (north) in km
    #
    x0,y0,un,ul = from_latlon(refll[0],refll[1])
    #
    lats,lons = utmtoll(xs+x0,ys+y0,un,force_zone_letter=ul)
    return lats,lons
#
