...
#
  ...
  if ($#argv < 4 || $#argv > 9) then
errormessage:
    echo ""
    echo "Usage: filter.csh master.PRM aligned.PRM filter decimation [rng_dec azi_dec] [compute_phase_gradient] [alpha, 0.5 in default] [fftsize, 32 in default]"
    echo ""
  ...
  echo "filter.csh with $#argv inputs..."
#
...
#
  ...
  #
  # not sure why we have below lines...
  if($#argv == 5) then
...
#
  ...
  # 
  #
  # Keep the original phase file before gaussian filtering...
  # 
  # echo " gmt grdmath imag.grd=bf real.grd=bf ATAN2 FLIPUD = phase_ori.grd"
  # gmt grdmath imag.grd=bf real.grd=bf ATAN2 FLIPUD = phase_ori.grd
  #
  # filter the real and imaginary parts of the interferogram
  # it seems that multilloking is done within below lines...
  #
...
#
  ...
  echo "  ... phasefilt -imag imagfilt.grd -real realfilt.grd -amp1 amp1.grd -amp2 amp2.grd -psize $fftsize -alpha $alpha"
  phasefilt -imag imagfilt.grd -real realfilt.grd -amp1 amp1.grd -amp2 amp2.grd -psize $fftsize -alpha $alpha 
